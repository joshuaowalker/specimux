...
def parse_args(argv):
    ...
    parser.add_argument("--resolve-multiple-matches", choices=[MultipleMatchStrategy.RETAIN, MultipleMatchStrategy.DOWNGRADE_FULL, MultipleMatchStrategy.DEREPLICATE], default=MultipleMatchStrategy.RETAIN, help="Strategy for handling multiple equivalent matches: 'retain' outputs all matches (default), 'downgrade-full' downgrades full matches to partial when multiple exist, 'dereplicate' groups by specimen and selects best match per specimen")
