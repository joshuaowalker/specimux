...
- Shares its primer searches with end matching (a per-read cache), so orientation detection costs no extra alignments
- Can disable with --disable-preorient

#### Lightweight Record Handling (v0.8)
- Input is parsed into minimal read records rather than Bio.SeqRecord objects, and quality travels as the raw phred string end to end
- Cheaper to parse, cheaper to send to worker processes, cheaper to write
