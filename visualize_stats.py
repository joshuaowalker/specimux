...
The tool automatically detects the format of the input JSON:
- Raw stats format: Contains 'unified_stats' with tuple keys converted to strings
- Flow format: Contains pre-processed 'nodes' and 'links' for direct visualization

Raw stats are converted to flow format on-the-fly, providing clean separation
between data processing (specimux.py) and visualization (this script).
